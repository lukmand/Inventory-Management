...
def calculate_in(df):
    ...
    update_query = '''
        ...
        from (
            select [Product ID], [Unit ID], sum(Quantity) Stock
            from df
            where [Transaction] = "IN"
            group by [Product ID], [Unit ID]
        ) x
...
def calculate_out(df):
    ...
    out_query = '''
        ...
        group by [Product ID], [Unit ID]
    '''
    out_df = cursor.execute(out_query).fetchall()

def readjust_stock(df, inventory_url, conversion_url):
    client = data.init_gsheet()
    ...
    trans_query = '''
        ...
        left join "{conversion_url}" y on x.[Product ID] = y.[Product ID] and x.[Unit ID] = y.[Original Unit ID] and y.[Convert Unit ID] = 3
        group by x.[Product ID]
    '''.format(conversion_url = conversion_url)
...
if submit:
    if vendor == "":
        ...
        data.gspread_write_data(gsheet_invoice, [invoiceid, datetime.datetime.now(), "PROCESS", vendor, invoiceprize])
